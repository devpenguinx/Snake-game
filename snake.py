...
def init_vars():
    global direction, head_pos, snake_body, food_pos, food_spawn, score
    direction = "RIGHT"
    head_pos = [120, 60]
    snake_body = [[120, 60]]
    food_pos = [random.randrange(
        1, (frame_size_x // square_size)) * square_size, random.randrange(
        1, (frame_size_y // square_size)) * square_size]
    food_spawn = True
    score = 0
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if ((event.key == pygame.K_DOWN or event.key == ord('s')) and (direction != "UP")):
                direction = "DOWN"
            elif ((event.key == pygame.K_UP or event.key == ord('w')) and (direction != "DOWN")):
                direction = "UP"
